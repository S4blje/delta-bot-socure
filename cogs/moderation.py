...
class Moderation(commands.Cog): 
  ...
  async def reactionmute(self, ctx, target: discord.Member, duration: int):
      ...
      if not muted_role:
          muted_role = await ctx.guild.create_role(name="Delta Reaction Mute")
        
        # Apply the "Muted" role to the target user
      await target.add_roles(muted_role)
        
        # Notify the user and the server
      await ctx.send(f"{target.mention} has been muted from reacting for {duration} minutes.")
        
        # Wait for the specified duration and then remove the role
      await asyncio.sleep(duration * 60)
      await target.remove_roles(muted_role)
        
        # Notify when the mute is lifted
  ...
  async def reactionunmute(self, ctx, target: discord.Member):
        # Find the "Muted" role
      muted_role = discord.utils.get(ctx.guild.roles, name="Delta Reaction Mute")
        
      if muted_role and muted_role in target.roles:
            # Remove the "Muted" role from the target user
          await target.remove_roles(muted_role)
