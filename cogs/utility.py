...
class Utility(Cog):
    ...
    async def bio(self, ctx: Context, *, member: Union[Member, User]=None):
                ...
                async with session.get(api_url) as response:
                    if response.status == 200:
                        ...
                        if user_data:
                            ...
                            embed = discord.Embed(title=f"Instagram Profile Info: {username}", color=discord.Color.blue())
                        ...
                        await ctx.send("Failed to fetch Instagram data.")
